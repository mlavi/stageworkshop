...
_role_uuid=$(curl ${CURL_HTTP_OPTS} --user ${PRISM_ADMIN}:${PE_PASSWORD} -X POST --data '{"kind":"role","filter":"name==Project Admin"}' 'https://localhost:9440/api/nutanix/v3/roles/list' | jq -r '.entities[] | .metadata.uuid' | tr -d \")

# Get the PC Account UUIDs
log "Get PC Account  UUID"

_pc_account_uuid=$(curl ${CURL_HTTP_OPTS} --user ${PRISM_ADMIN}:${PE_PASSWORD} -X POST --data '{"kind":"account","filter":"type==nutanix_pc"}' 'https://localhost:9440/api/nutanix/v3/accounts/list' | jq -r '.entities[] | .status.resources.data.cluster_account_reference_list[0].resources.data.pc_account_uuid' | tr -d \")
